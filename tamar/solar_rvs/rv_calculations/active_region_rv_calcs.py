...
csv_name = os.path.join(CsvDir.ACTIVE, '02072021_native.csv')
bad_dates_csv = os.path.join(CsvDir.ACTIVE, 'bad_dates.csv')
...
# start_date = datetime.datetime(2019, 11, 11, 12, 0, 0)
start_date = datetime.datetime(2021, 2, 7, 00, 00, 00)
# end_date = datetime.datetime(2021, 9, 21, 00, 00, 00)
# dates_list = [start_date + datetime.timedelta(seconds=12*60*s) for s in range((end_date - start_date).seconds) if s%(60*12) == 0]
# dates_list = [start_date + datetime.timedelta(seconds=s) for s in range((end_date - start_date).second) if s%(45) == 0]
dates_list = [start_date + datetime.timedelta(seconds=45*s) for s in range(0, int(24*3600/45))]

for i, date in enumerate(dates_list[1691:]):
