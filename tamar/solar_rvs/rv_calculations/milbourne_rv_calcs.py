...
Pipeline to calculate and save values necessary for the calculation
of solar RV variations.

Calculations are done following the same methods as solar_corrections.ipynb.

Values are saved to a CSV value with their corresponding timestamp.

The calculation is done one timestamp at a time for the following reasons:
    1. ensure we have all three data products for each time stamp
    2. in case this crashes you don't lose everything!!
    3. takes up less memory
    4. probably other reasons I have yet to think of
...
csv_name = os.path.join(CsvDir.MILBOURNE, 'full_calcs.csv')

# name of csv to store bad dates
bad_dates_csv = os.path.join(CsvDir.MILBOURNE, 'bad_dates.csv')

# create list of dates
data_csv = os.path.join(CsvDir.MILBOURNE, 'milbourne_data_full.csv')
data_df = pd.read_csv(data_csv)
dates_list = data_df.date_obs.values
mil_f = data_df.f.values
mil_rv = data_df.rv_sun.values
mil_model = data_df.rv_model.values
vphot_ref = data_df.v_phot.values
vconv_ref = data_df.v_conv.values

# print out csv title
print("Beginning calculation of values for csv file: " + csv_name)

# List of header strings
row_contents = ['date_obs', 'date_jd', 'rv_sun', 'rv_model_ref', 'f_ref', 'v_phot_ref', 'v_conv_ref', 'v_quiet', 'v_disc', 'v_phot', 'v_conv', 'f_bright', 'f_spot', 'f', 'Bobs',
...
for i, date in enumerate(dates_list[550:]):
    ...
    if len(good_files) != 3:
        ...
        if missing_map:
            ...
            quiet_flux, ar_flux, conv_flux, pol_flux, pol_conv_flux = sfuncs.area_unsigned_flux(map_mag_obs, imap, area,
                                                                                                active)
            ...
            save_vals = [v_quiet,  v_disc, v_phot, v_conv, f_bright, f_spot, f, unsigned_obs_flux, vphot_bright,
                         vphot_spot, f_small, f_large, f_network, f_plage, f_nonconv, quiet_flux, ar_flux, conv_flux,
                         pol_flux, pol_conv_flux, vconv_quiet, vconv_large, vconv_small]

            # round stuff
            rounded = np.around(save_vals, 3)
            round_vals = [date_str, date_jd, mil_rv[i], mil_model[i], mil_f[i], vphot_ref[i], vconv_ref[i]]
            for val in rounded:
...
# csv_ref = pd.read_csv(csv_name)
# dates = csv_ref.date_jd.values
# test = np.isin(dates_list, dates)
# csv_ref["v_phot_ref"] = vphot_ref[test]
# csv_ref["v_conv_ref"] = vconv_ref[test]
#
# csv_ref.to_csv(csv_name, index=False)
