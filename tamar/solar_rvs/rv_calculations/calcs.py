...
csv_name = os.path.join(CsvDir.NEID_CALC, 'jenn_plage1.csv')
bad_dates_csv = os.path.join(CsvDir.MILBOURNE, 'bad_dates_1719.csv')
...
start_date = datetime.datetime(2021, 8, 11, 12, 15, 0)
end_date = datetime.datetime(2021, 8, 12, 0, 00, 0)
dates_list = [start_date + datetime.timedelta(seconds=15*60*x) for x in range(0, 192)]
# dates_list = [start_date + datetime.timedelta(days=x) for x in range((end_date - start_date).days)]

# for i, date in enumerate(dates_list[846:]):
for i, date in enumerate(dates_list[11:]):
    ...
    if len(good_files) != 3:
        ...
        if missing_map:
            ...
            save_vals = [v_quiet, v_disc, v_phot, v_conv, f_bright, f_spot, f, unsigned_obs_flux, vphot_bright,
                         vphot_spot, f_small, f_large, f_network, f_plage, f_nonconv]
                # , quiet_flux, ar_flux,
